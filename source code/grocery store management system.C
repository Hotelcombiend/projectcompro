...
//				 break;
			...
			gotoxy(11,34);
			printf("Plese Enter right character ONLY (S,R,B,A,E).");
			getch();
		}
	}while(ch!='E');
}
...
void supplier()
{
  char ch;
	do
	{
	  ...
	  gotoxy(10,40);
	  //textcolor(15);
	  printf("Press the number for further Operations  ");
	  ...
	  switch(ch)
	  {
		...
		default://textcolor(4+BLINK);
			gotoxy(11,34);
			printf("Plese Enter right character ONLY (1,2,3,4,5).");
			getch();
	  }
	}while(ch!='M');
...
void report_menu()
{
	...
	do
	{
	  ...
	  switch(ch)
	  {
		...
		default://textcolor(4+BLINK);
			gotoxy(11,34);
			printf("Plese Enter right character ONLY (1,2,3,4,5,6).");
			getch();
	  }
	}while(ch!='M');
...
//   window(1,1,80,50);
	...
	gotoxy(10,8);
	printf("=> This project is about grocery store ");
	gotoxy(10,10);
	printf("=> In this project we can add a product");
	gotoxy(10,12);
	printf("=> We can modifed & delete the existing product");
	gotoxy(10,14);
	printf("=> We can also search for product and checked bill");
	gotoxy(10,16);
	printf("=> It's helpful for calculating the profit for the store");
	gotoxy(10,18);
	printf("=> And it is convenient for the owner of the store");
	gotoxy(10,20);
...
//		printf("Cust_ID    Cust_Name");
		...
		while((fread(&temp,sizeof(temp),1,ptr))==1)
		{
			...
			else if((strcmp(temp.id,tar)==0))
			{
				...
				gotoxy(8,10);
				printf(" Product Name          : %s",temp.medi_name);
				gotoxy(8,12);
...
void sale_rpt()
{
  ...
  system("cls");
	   ...
	   while(fscanf(ptrs_r,"%s %s %s %d %f %f %d %d %d\n",s_r.medi_id,s_r.medir_name,s_r.cust_name,&s_r.qty,&s_r.rate,&s_r.total,&s_r.sDay,&s_r.sMonth,&s_r.sYear)!=EOF)
	   {
		...
		printf("%.2f",s_r.total);
		gotoxy(65,j);
		printf("%d-%d-%d",s_r.sDay,s_r.sMonth+1,s_r.sYear+1900);
		j=j+2;
	   }
	   getche();
}
...
void pur_rpt()
{
  ...
  box();
	   ...
	   while(fscanf(ptrp_r,"%s %s %s %d %f %f %d %d %d\n",p_r.medi_id,p_r.medir_name,p_r.supp_name,&p_r.qty,&p_r.rate,&p_r.total,&p_r.sDay,&p_r.sMonth,&p_r.sYear)!=EOF)
	   {
		...
		printf("%.2f",p_r.total);
		gotoxy(65,j);
		printf("%d-%d-%d",p_r.sDay,p_r.sMonth+1,p_r.sYear+1900);
		j+=2;
	   }
	   getche();
}
...
void profit_rpt()
{
  ...
  box();
	   ...
	   while(fscanf(ptrpr_r,"%s %s %d %d %d %d %f %f %f \n",pr_r.medi_id,pr_r.medir_name,&pr_r.sDay,&pr_r.sMonth,&pr_r.sYear,&pr_r.qty,&pr_r.unit,&pr_r.rate,&pr_r.profit)!=EOF)
	   {
		...
		gotoxy(28,j);
		printf("%d-%d-%d",pr_r.sDay,pr_r.sMonth+1,pr_r.sYear+1900);
		gotoxy(40,j);
		printf("%d",pr_r.qty);
		gotoxy(48,j);
		printf("%.2f",pr_r.unit);
